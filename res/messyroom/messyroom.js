...
let updatePerspective = (originX, originY) => {
  ...
  viewSpan.style.perspectiveOrigin =
    originX.toFixed(4) + "% " + originY.toFixed(4) + "%";
  viewSpan.style.webkitPerspectiveOrigin =
    originX.toFixed(4) + "% " + originY.toFixed(4) + "%";
};
...
let displaySuccessMessage = () => {
  let introDiv = document.getElementById("c-body");
  let successCode =
    '<h2 class="c-scroll__heading">Look around!!!</h2><br><p class="c-scroll__paragraph">Dyuksha\'s On...</p> ' +
...
let redirect = () => {
  setTimeout(() => {
    window.location.href = "../imagePuzzle/imagePuzzle.html";
  }, 1000);
};
