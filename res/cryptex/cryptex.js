...
var ringElements;
var ring;
const ringPrefix = "ring";

document.addEventListener("DOMContentLoaded", () => {
  container = document.getElementById('cryptex_container');
  stage = document.getElementById('stage');
  codeElement = document.getElementById('code');
  charElements = codeElement.getElementsByClassName('char');
  obj = document.getElementById('object');
  ringElements = obj.getElementsByClassName('ring');
  ring = document.getElementById('ring1');
  initialize();
});
...
var hint = "";
let introCode = "welcometodavincicode";
var solution = "GREATTHINGSARECOMING";
for (let i = 0; i < numRings; ++i)  hint += " ";
if (introCode.length < numRings){
    let leftPadding = Math.floor((numRings - introCode.length - 1)/2);
    let rightPadding = leftPadding + introCode.length;
    let tempIntroCode = introCode;
    introCode = "";
    for (var i = introCode.length - 1; i < numRings; ++i)  {
        if(i < leftPadding || i >= rightPadding) {
            introCode += "I";
        } else if(i < rightPadding) {
          introCode += tempIntroCode[i - leftPadding];
        }
    }
}
...
function drawCodeRings() {
  for (var i = 0; i < numRings; i++) {
    ...
    rings[i] = drawRing(i);
    rings[i].name = ringPrefix + String(i + 1);
    rings[i].rotationX = 0;
...
function cryptexInitialSetting(){
  let i;
  let setCode = introCode.toUpperCase();
  for (i = 0; i < introCode.length; ++i) {
      onKeyPress({
          keyCode: setCode.charCodeAt(i)
      });
  }
  while (i-- > 0) selectPreviousRing();
  // selectNextRing();
}
