...
let moveCamel = (camel, frameRate) => {
    let containerWidth = parseInt(document.getElementById("container").getBoundingClientRect().width);
...
let loadCamels = () => {
    ...
    camel1.src = "img/camel1.gif";
    camel2.src = "img/camel2.gif";
}
...
let checkRoute = (keyCode) => {
    ...
    if (key === "I") {
        ...
        if(i < code.length) {
            if (key === code[i++]) {
                input += key;
                let r = presentFocus.i, c = presentFocus.j;
